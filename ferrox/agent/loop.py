...
class AgentLoop:
    ...
    async def execute_task_with_test_loop(
        self, task_description: str, test_command: str = None, cwd: str = "."
    ) -> dict:
        """
        ...
        while attempts < self.max_retries:
            ...
            try:
                ...
                if test_command:
                    ...
                    test_result_str = execute_tool(
                        "run_command", {"command": test_command, "cwd": cwd}
                    )
                    ...
                    return {
                        "success": True,
                        "message": "Task completed (no tests run).",
                        "attempts": attempts,
                    }
        ...
        return {
            "error": f"Max retries ({self.max_retries}) reached. Last error: {last_error}",
            "attempts": attempts,
        }
