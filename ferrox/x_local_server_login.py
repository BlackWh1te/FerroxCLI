"""X (Twitter) login via local HTTP server + real browser.

Instead of launching an automated browser (blocked by X bot detection),
this approach:
...
4. User logs in to X in their real browser, clicks the bookmarklet
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .step-num{display:inline-block;background:#00d4ff;color:#0f0f1a;
  ...
  .btn{display:inline-block;background:#00d4ff;color:#0f0f1a;border:none;
...
n  .btn-secondary:hover{background:#3a3a5e}
...
<body>
<h1>🐦 Ferrox X Login</h1>
<p class="subtitle">Log in to X using your real browser — no automation detected</p>

<div class="step">
  <span class="step-num">1</span>
  <h3>Open X in your real browser</h3>
  <p>Click the button below to open <b>x.com</b> in a new tab. Log in normally with
     your username, password, and any 2FA.</p>
  <a class="btn" href="https://x.com/i/flow/login" target="_blank">Open x.com/login</a>
</div>
...
<div class="step">
  ...
  <textarea id="cookies" placeholder='[{"name":"auth_token","value":"...","domain":".x.com"},...]'></textarea>
  <br>
        ...
        if self.path == "/save":
            try:
                ...
                _CookieHandler.result_msg = (
                    ...
                    f"You can close this tab and run '/social start'."
                )
...
async def x_login_via_local_server(timeout_seconds: int = 300) -> str:
    ...
    The user opens the printed URL in their real browser, logs in to X,
    copies cookies via the provided bookmarklet or DevTools, pastes them
    into the form, and the server saves them for twikit reuse.
    ...
    # Print instructions (rich console may not be available here, use print)
    print("\n" + "=" * 62)
    print("  🐦  X Login via Real Browser")
    print("=" * 62)
    ...
    print("    1. Open x.com/login and log in normally")
