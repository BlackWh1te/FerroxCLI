...
def show_trace_viewer(agent_logs: list):
    ...
    if not agent_logs:
        ...
        successes = sum(1 for log in agent_logs if log['type'] == 'tool_result' and log.get('success'))
        failures = sum(1 for log in agent_logs if log['type'] == 'tool_result' and not log.get('success'))
        
        console.print(f"[dim]Summary:[/dim] {thoughts} thoughts | {tool_calls} tool calls | {successes} ✅ | {failures} ❌\n")
        console.print("-" * 60 + "\n")
